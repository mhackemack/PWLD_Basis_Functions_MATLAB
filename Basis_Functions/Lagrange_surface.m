...
elseif dim == 2
	...
	[qx,  qw, ndim] = get_2D_surface_ref_terms(ctype, fverts, qqx, qw);
    qqx = qx;
    nqx = length(qw);
elseif dim == 3
    sArea = polygonArea3d(verts(fverts,:));
    [qx, qqx, qw, ndim] = get_3D_surface_ref_terms(verts, fverts, ctype, deg);
    nqx = length(qw);
end
if ctype == 1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [xout, qw, ndim] = get_2D_surface_ref_terms(ctype, fverts, pts_1D, qw)
if ctype == 1
    switch(fverts(1))
        case(1)
